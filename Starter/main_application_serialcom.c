...
void main_demo(void) {
	...
	init_7seg_comm();
	init_serial_uplink(COM_CH0);
	init_serial_downlink(COM_CH0);
	...
	// TASKS 
	xTaskCreate(SerialSend_Task_0, "STx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAL_SEND_PRI, NULL);	// SERIAL TRANSMITTER TASK 
	xTaskCreate(SerialReceive_Task_0, "SRx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAl_REC_PRI, NULL);	// SERIAL RECEIVER TASK 
	xTaskCreate(SerialSend_Task_1, "STx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAL_SEND_PRI, NULL);	// SERIAL TRANSMITTER TASK 
	xTaskCreate(SerialReceive_Task_1, "SRx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAl_REC_PRI, NULL);	// SERIAL RECEIVER 
...
void SerialSend_Task_0(void* pvParameters) {
	t_point = 0;
	while (1) {
		if (t_point > (sizeof(trigger) - 1))
			t_point = 0;
		send_serial_character(COM_CH0, trigger[t_point++]);
...
void SerialSend_Task_1(void* pvParameters) {
	t_point = 0;
	while (1) {
		if (t_point > (sizeof(trigger) - 1))
			t_point = 0;
		send_serial_character(COM_CH1, trigger[t_point++]);
		xSemaphoreTake(TBE_BinarySemaphore, portMAX_DELAY);// kada se koristi predajni interapt
		//vTaskDelay(pdMS_TO_TICKS(100));// kada se koristi vremenski delay
	}
}
...
void SerialReceive_Task_0(void* pvParameters) {
	uint8_t cc = 0;
...
void SerialReceive_Task_1(void* pvParameters) {
	uint8_t cc = 0;
	static uint8_t loca = 0;
	while (1) {
		xSemaphoreTake(RXC_BinarySemaphore, portMAX_DELAY);// ceka na serijski prijemni interapt
		get_serial_character(COM_CH1, &cc);//ucitava primljeni karakter u promenjivu cc
		printf("primio karakter: %u\n", (unsigned)cc);// prikazuje primljeni karakter u cmd prompt
